...
int auto_find_ini(char *findDir, char *format, char *inipath)
{
	...
	snprintf((char *)tmp, 35, "/vendor/firmware/test_sensor_%d.ini", gt1x_version.sensor_id);
...
/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/17 start */
static ssize_t zh_gtp_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	unsigned char *result = NULL;
	char *ito_buf = NULL;
	u32 ito_result = 0;
	...
	if (ito_result == _TEST_PASS) {
		count = sprintf(buf, "%d\n", 1);//return test  result
	} else {
		count = sprintf(buf, "%d\n", 0);//return test  result
	}
...
static ssize_t zh_gtp_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(factory_check, 0644, zh_gtp_test_show, zh_gtp_test_store);
...
int gtp_test_node_init(struct platform_device *tpinfo_device)
{
    int err=0;
    err = sysfs_create_group(&tpinfo_device->dev.kobj, &zh_test_attribute_group);
    if (0 != err)
    {
        GTP_ERROR( "ERROR: ITO node create failed.");
        sysfs_remove_group(&tpinfo_device->dev.kobj, &zh_test_attribute_group);
        return -EIO;
    }
    else
    {
        GTP_INFO("ITO node create_succeeded.");
    }
    return err;
}

int gtp_test_node_exit(struct platform_device *tpinfo_device)
{
	sysfs_remove_group(&tpinfo_device->dev.kobj, &zh_test_attribute_group);
	return 0;
}
